...
public class PersonEndpoints : IEndpoints
{
    private const string ContentType = "application/json";
    private const string Tag = "Persones";
    private const string BaseRoute = "Persones";
    ...
    {
        // Create Endpoints
        app.MapPost(BaseRoute, CreatePersonModelAsync)
            .WithName("CreatePerson")
            .Accepts<EditiblePersonModel>(ContentType)
            .Produces<PersonModel>(201)
            .Produces<IEnumerable<ValidationFailure>>(400)
            .WithTags(Tag);

        // Read Endpoints
        app.MapGet(BaseRoute, GetAllPersonModelsAsync)
            .WithName("GetAllPersonModels")
            .Produces(200)
            .WithTags(Tag);

        app.MapGet($"{BaseRoute}/{{id}}", GetPersonModelByIDAsync)
        ...
        app.MapGet($"{BaseRoute}/entry{{entryId}}", GetPersonModelByEntryIDAsync)
            .WithName("GetPersonModelByEntryID")
            .Produces(200)
            .Produces<IEnumerable<ValidationFailure>>(400)      // you gave bad info
            .Produces(404)                                      // could not find entry to update
            .WithTags(Tag);

        //app.MapGet($"{BaseRoute}/history{{id}}", GetHistroyOfPersonModelByIDAsync)
        //    .WithName("GetHistroyOfPersonModelByID")
        //    .WithTags(Tag);

        // Update Endpoints
        app.MapPut($"{BaseRoute}/{{id}}", UpdatePersonModelAsync)
            .WithName("UpdatePersonModel")
            .Accepts<EditiblePersonModel>(ContentType)
            .Produces<PersonModel>(200)
            .Produces<IEnumerable<ValidationFailure>>(400)      // you gave bad info
            .Produces(404)                                      // could not find entry to update
            .WithTags(Tag);
    ...
    /// </summary>
    /// <param name="newEntry">The parameters used to make the new Person Model</param>
    /// <param name="service">The service class the serves this endpoint for database operations</param>
    /// <param name="vaildator">Person Model value validator</param>
    ...
    private static async Task<IResult> CreatePersonModelAsync(EditiblePersonModel newEntry,
        IPersonService service, IValidator<EditiblePersonModel> vaildator,
        LinkGenerator linker, HttpContext http, ResumeContext ctx)
    {
        ...
        //{
        //    new ("ID", "This Person already exists in the database." ),
        //});
        return created.Match(
            succ =>
            {
                var locationUri = linker.GetUriByName(http, "GetPersonModel", new { id = succ.CommonIdentity });
    ...
    private static async Task<IResult> GetAllPersonModelsAsync(IPersonService service, ResumeContext ctx, string? nameSearchTerm,
    ...
    /// <response code="400">Something went wrong or the database does not exist</response>
    /// <response code="404">If the id is not found in the database</response>
    private static async Task<IResult> GetPersonModelByIDAsync(int id, IPersonService service, ResumeContext ctx)
    {
    ...
    /// <response code="400">Something went wrong or the database does not exist</response>
    /// <response code="404">If the id is not found in the database</response>
    private static async Task<IResult> GetPersonModelByEntryIDAsync(int entryId,IPersonService service, ResumeContext ctx)
    {
    ...
    //private static async Task<IResult> GetHistroyOfPersonModelByIDAsync(int id, IPersonService service, ResumeContext ctx)
    //{
    ...
    /// <summary>
    /// Search for and remove a specific Person Model entry from the database
    /// </summary>
    /// <param name="id">TODO</param>
    ...
    /// <response code="400">Something went wrong or the database does not exist</response>
    /// <response code="404">If the id is not found in the database</response>
    private static async Task<IResult> UpdatePersonModelAsync(int id, EditiblePersonModel changes,
        IPersonService service, IValidator<EditiblePersonModel> vaildator, ResumeContext ctx)
    {
    ...
    /// <response code="400">Something went wrong or the database does not exist</response>
    /// <response code="404">If the id is not found in the database</response>
    private static async Task<IResult> DeleteAsync(int id, IPersonService service, ResumeContext ctx)
    {
    ...
    /// <response code="400">Something went wrong or the database does not exist</response>
    /// <response code="404">If the id is not found in the database</response>
    private static async Task<IResult> DeleteEntryAsync(int id, IPersonService service, ResumeContext ctx)
    {
