...
public class PhoneNumberEndpoints : IEndpoints
{
    ...
    {
        // Create Endpoints
        app.MapPost(BaseRoute, CreatePhoneNumberModelAsync)
            .WithName("CreatePhoneNumber")
            .Accepts<EditiblePhoneNumberModel>(ContentType)
            .Produces<PhoneNumberModel>(201)
            .Produces<IEnumerable<ValidationFailure>>(400)
            .WithTags(Tag);

        // Read Endpoints
        app.MapGet(BaseRoute, GetAllPhoneNumberModelsAsync)
            .WithName("GetAllPhoneNumberModels")
            .Produces(200)
            .WithTags(Tag);

        app.MapGet($"{BaseRoute}/{{id}}", GetPhoneNumberModelByIDAsync)
        ...
        app.MapGet($"{BaseRoute}/entry{{entryId}}", GetPhoneNumberModelByEntryIDAsync)
            .WithName("GetPhoneNumberModelByEntryID")
            .Produces(200)
            .Produces<IEnumerable<ValidationFailure>>(400)      // you gave bad info
            .Produces(404)                                      // could not find entry to update
            .WithTags(Tag);

        //app.MapGet($"{BaseRoute}/history{{id}}", GetHistroyOfPhoneNumberModelByIDAsync)
        //    .WithName("GetHistroyOfPhoneNumberModelByID")
        //    .WithTags(Tag);

        // Update Endpoints
        app.MapPut($"{BaseRoute}/{{id}}", UpdatePhoneNumberModelAsync)
            .WithName("UpdatePhoneNumberModel")
            .Accepts<EditiblePhoneNumberModel>(ContentType)
            .Produces<PhoneNumberModel>(200)
            .Produces<IEnumerable<ValidationFailure>>(400)      // you gave bad info
            .Produces(404)                                      // could not find entry to update
            .WithTags(Tag);
    ...
    /// </summary>
    /// <param name="newEntry">The parameters used to make the new PhoneNumber Model</param>
    /// <param name="service">The service class the serves this endpoint for database operations</param>
    /// <param name="vaildator">PhoneNumber Model value validator</param>
    ...
    private static async Task<IResult> CreatePhoneNumberModelAsync(EditiblePhoneNumberModel newEntry,
        IPhoneNumberService service, IValidator<EditiblePhoneNumberModel> vaildator,
        LinkGenerator linker, HttpContext http, ResumeContext ctx)
    {
        ...
        //{
        //    new ("ID", "This PhoneNumber already exists in the database." ),
        //});
        return created.Match(
            succ =>
            {
                var locationUri = linker.GetUriByName(http, "GetPhoneNumberModel", new { id = succ.CommonIdentity });
    ...
    private static async Task<IResult> GetAllPhoneNumberModelsAsync(IPhoneNumberService service, ResumeContext ctx, string? nameSearchTerm,
    ...
    /// <response code="400">Something went wrong or the database does not exist</response>
    /// <response code="404">If the id is not found in the database</response>
    private static async Task<IResult> GetPhoneNumberModelByIDAsync(int id, IPhoneNumberService service, ResumeContext ctx)
    {
    ...
    /// <response code="400">Something went wrong or the database does not exist</response>
    /// <response code="404">If the id is not found in the database</response>
    private static async Task<IResult> GetPhoneNumberModelByEntryIDAsync(int entryId,IPhoneNumberService service, ResumeContext ctx)
    {
    ...
    //private static async Task<IResult> GetHistroyOfPhoneNumberModelByIDAsync(int id, IPhoneNumberService service, ResumeContext ctx)
    //{
    ...
    /// <summary>
    /// Search for and remove a specific PhoneNumber Model entry from the database
    /// </summary>
    /// <param name="id">TODO</param>
    ...
    /// <response code="400">Something went wrong or the database does not exist</response>
    /// <response code="404">If the id is not found in the database</response>
    private static async Task<IResult> UpdatePhoneNumberModelAsync(int id, EditiblePhoneNumberModel changes,
        IPhoneNumberService service, IValidator<EditiblePhoneNumberModel> vaildator, ResumeContext ctx)
    {
    ...
    /// <response code="400">Something went wrong or the database does not exist</response>
    /// <response code="404">If the id is not found in the database</response>
    private static async Task<IResult> DeleteAsync(int id, IPhoneNumberService service, ResumeContext ctx)
    {
    ...
    /// <response code="400">Something went wrong or the database does not exist</response>
    /// <response code="404">If the id is not found in the database</response>
    private static async Task<IResult> DeleteEntryAsync(int id, IPhoneNumberService service, ResumeContext ctx)
    {
