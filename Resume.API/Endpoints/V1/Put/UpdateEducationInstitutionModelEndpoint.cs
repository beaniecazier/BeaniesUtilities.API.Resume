...
/// </summary>
[ApiVersion(1.0)]
public class UpdateEducationInstitutionModelEndpoint : IEndpoints
{
    ...
    public static void DefineEndpoints(IEndpointRouteBuilder app)
    {
        ...
        // Update Endpoints
        Log.Information("Now adding EducationInstitution Model put endpoints");
        var singleEndpoint = app.MapPut($"{EndpointPrefix}/{{id}}", UpdateEducationInstitutionModelAsync)
            .WithName("UpdateEducationInstitutionModel")
            .Accepts<EducationInstitutionModel>(ContentType)
            .Produces<EducationInstitutionModel>(StatusCodes.Status200OK)
    ...
    /// <summary>
    /// Search for and remove a specific EducationInstitution Model result from the database
    /// </summary>
        ...
        IEducationInstitutionModelService service, IAddressModelService addressService, ICertificateModelService certificateService, IEducationDegreeModelService educationDegreeService, LinkGenerator linker, HttpContext http, CancellationToken token)
    {
        ...
        Log.Information("Update EducationInstitution Model endpoint called by {username}", @username);

        var oldModel = await service.GetByIDAsync(changes.Id, token);
        if (oldModel.IsFail)
        {
            Log.Error(((Error)oldModel).ToException(), "Server issue encountered while trying to get all EducationInstitution Models from the database");
        ...
        {
            Log.Error("EducationInstitution Model with ID:{id} does not exist", @changes.Id);
            return Results.NotFound();
        }
        
        var requestedAddressModel = await addressService.GetByIDAsync(changes.Address.Value, token);
        ...
        var requestedCertificateModels = await certificateService.GetAllAsync(new GetAllCertificateModelsOptions {SpecificIds = changes.CertificatesIssued}, token);
        ...
        var requestedEducationDegreeModels = await educationDegreeService.GetAllAsync(new GetAllEducationDegreeModelsOptions {SpecificIds = changes.DegreesGiven}, token);
        ...
        var newModel = changes.MapToModelFromUpdateRequest((EducationInstitutionModel)oldModel!, username, (AddressModel)requestedAddressModel, (List<CertificateModel>)requestedCertificateModels, (List<EducationDegreeModel>)requestedEducationDegreeModels);
        ...
        var result = await service.UpdateAsync(newModel, (EducationInstitutionModel)oldModel, token);
        return result.Match(
            Succ =>
            {
                Log.Information("EducationInstitution Model with id {model.CommonIdentity} was successfully updated", @newModel.CommonIdentity);
                var locationUri = linker.GetUriByName(http, "GetEducationInstitutionModelByID", new { id = newModel.CommonIdentity });
