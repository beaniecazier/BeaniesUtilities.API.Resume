...
/// <summary>
/// Service class for Project Model to handle CRUD operations with database
/// </summary>
public class ProjectModelService : IProjectModelService
{
    ...
    private readonly IValidator<GetAllProjectModelsOptions> _optionsValidator;
    private readonly IProjectModelRepository _repository;
    ...
    public ProjectModelService(
        IProjectModelRepository repository,
        IValidator<ProjectModel> validator,
        IValidator<GetAllProjectModelsOptions> optionsValidator)
    {
    ...
    public async Task<int> GetQueryTotal(GetAllProjectModelsOptions options) => await _repository.GetQueryTotal(options);

    public async Task<IEnumerable<ValidationFailure>> ValidateModelForCreation(ProjectModel model)
    {
    ...
    public async Task<IEnumerable<ValidationFailure>> ValidateGetAllModelOptions(GetAllProjectModelsOptions options)
    {
    ...
    public async Task<IEnumerable<ValidationFailure>> ValidateModelForUpdate(ProjectModel model)
    {
        var modelValidationResult = await _validator.ValidateAsync(model);
        if (!modelValidationResult.IsValid) return modelValidationResult.Errors;
        return Enumerable.Empty<ValidationFailure>();
    ...
    public async Task<Fin<int>> CreateAsync(ProjectModel model, CancellationToken token = default)
    {
    ...
    /// </summary>
    /// <returns>A list of all Project Models or the fail conditions</returns>
    public async Task<Fin<IEnumerable<ProjectModel>>> GetAllAsync(GetAllProjectModelsOptions options, CancellationToken token = default)
    {
    ...
    public async Task<Fin<int>> UpdateAsync(ProjectModel newModel, ProjectModel oldModel, CancellationToken token = default)
    {
