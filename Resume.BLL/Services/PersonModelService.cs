...
/// <summary>
/// Service class for Person Model to handle CRUD operations with database
/// </summary>
public class PersonModelService : IPersonModelService
{
    ...
    private readonly IValidator<GetAllPersonModelsOptions> _optionsValidator;
    private readonly IPersonModelRepository _repository;
    ...
    public PersonModelService(
        IPersonModelRepository repository,
        IValidator<PersonModel> validator,
        IValidator<GetAllPersonModelsOptions> optionsValidator)
    {
    ...
    public async Task<int> GetQueryTotal(GetAllPersonModelsOptions options) => await _repository.GetQueryTotal(options);

    public async Task<IEnumerable<ValidationFailure>> ValidateModelForCreation(PersonModel model)
    {
    ...
    public async Task<IEnumerable<ValidationFailure>> ValidateGetAllModelOptions(GetAllPersonModelsOptions options)
    {
    ...
    public async Task<IEnumerable<ValidationFailure>> ValidateModelForUpdate(PersonModel model)
    {
        var modelValidationResult = await _validator.ValidateAsync(model);
        if (!modelValidationResult.IsValid) return modelValidationResult.Errors;
        return Enumerable.Empty<ValidationFailure>();
    ...
    public async Task<Fin<int>> CreateAsync(PersonModel model, CancellationToken token = default)
    {
    ...
    /// </summary>
    /// <returns>A list of all Person Models or the fail conditions</returns>
    public async Task<Fin<IEnumerable<PersonModel>>> GetAllAsync(GetAllPersonModelsOptions options, CancellationToken token = default)
    {
    ...
    public async Task<Fin<PersonModel>> GetByIDAsync(int id, CancellationToken token = default)
    {
        var model = await _repository.TryGetByIdAsync(id, token);
        if(((PersonModel)model).IsNull()) return Error.New(new NullReferenceException());
        return model;
    }
    ...
    public async Task<Fin<int>> UpdateAsync(PersonModel newModel, PersonModel oldModel, CancellationToken token = default)
    {
    ...
    public async Task<Fin<PersonModel>> DeleteAsync(int id, CancellationToken token = default)
    {
        var lastCopy = await _repository.TryGetByIdAsync(id, token);
        if (((PersonModel)lastCopy).IsNull()) return Error.New(new NullReferenceException());
