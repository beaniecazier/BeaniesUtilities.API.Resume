...
/// <summary>
/// Service class for WorkExperience Model to handle CRUD operations with database
/// </summary>
public class WorkExperienceModelService : IWorkExperienceModelService
{
    #region Fields

    private readonly IValidator<WorkExperienceModel> _validator;
    private readonly IValidator<GetAllWorkExperienceModelsOptions> _optionsValidator;
    private readonly IWorkExperienceModelRepository _repository;

    #endregion

    #region Constructors

    public WorkExperienceModelService(
        IWorkExperienceModelRepository repository,
        IValidator<WorkExperienceModel> validator,
        IValidator<GetAllWorkExperienceModelsOptions> optionsValidator)
    {
    ...
    public async Task<int> GetQueryTotal(GetAllWorkExperienceModelsOptions options) => await _repository.GetQueryTotal(options);

    public async Task<IEnumerable<ValidationFailure>> ValidateModelForCreation(WorkExperienceModel model)
    {
    ...
    public async Task<IEnumerable<ValidationFailure>> ValidateGetAllModelOptions(GetAllWorkExperienceModelsOptions options)
    {
    ...
    public async Task<IEnumerable<ValidationFailure>> ValidateModelForUpdate(WorkExperienceModel model)
    {
        var modelValidationResult = await _validator.ValidateAsync(model);
        if (!modelValidationResult.IsValid) return modelValidationResult.Errors;
        return Enumerable.Empty<ValidationFailure>();
    ...
    public async Task<Fin<int>> CreateAsync(WorkExperienceModel model, CancellationToken token = default)
    {
    ...
    /// </summary>
    /// <returns>A list of all WorkExperience Models or the fail conditions</returns>
    public async Task<Fin<IEnumerable<WorkExperienceModel>>> GetAllAsync(GetAllWorkExperienceModelsOptions options, CancellationToken token = default)
    {
    ...
    public async Task<Fin<int>> UpdateAsync(WorkExperienceModel newModel, WorkExperienceModel oldModel, CancellationToken token = default)
    {
