...
/// <summary>
/// Service class for PhoneNumber Model to handle CRUD operations with database
/// </summary>
public class PhoneNumberModelService : IPhoneNumberModelService
{
    #region Fields

    private readonly IValidator<PhoneNumberModel> _validator;
    private readonly IValidator<GetAllPhoneNumberModelsOptions> _optionsValidator;
    private readonly IPhoneNumberModelRepository _repository;

    #endregion

    #region Constructors

    public PhoneNumberModelService(
        IPhoneNumberModelRepository repository,
        IValidator<PhoneNumberModel> validator,
        IValidator<GetAllPhoneNumberModelsOptions> optionsValidator)
    {
    ...
    public async Task<int> GetQueryTotal(GetAllPhoneNumberModelsOptions options) => await _repository.GetQueryTotal(options);

    public async Task<IEnumerable<ValidationFailure>> ValidateModelForCreation(PhoneNumberModel model)
    {
    ...
    public async Task<IEnumerable<ValidationFailure>> ValidateGetAllModelOptions(GetAllPhoneNumberModelsOptions options)
    {
    ...
    public async Task<IEnumerable<ValidationFailure>> ValidateModelForUpdate(PhoneNumberModel model)
    {
        var modelValidationResult = await _validator.ValidateAsync(model);
        if (!modelValidationResult.IsValid) return modelValidationResult.Errors;
        return Enumerable.Empty<ValidationFailure>();
    ...
    public async Task<Fin<int>> CreateAsync(PhoneNumberModel model, CancellationToken token = default)
    {
    ...
    /// </summary>
    /// <returns>A list of all PhoneNumber Models or the fail conditions</returns>
    public async Task<Fin<IEnumerable<PhoneNumberModel>>> GetAllAsync(GetAllPhoneNumberModelsOptions options, CancellationToken token = default)
    {
    ...
    public async Task<Fin<PhoneNumberModel>> GetByIDAsync(int id, CancellationToken token = default)
    {
        var model = await _repository.TryGetByIdAsync(id, token);
        if(((PhoneNumberModel)model).IsNull()) return Error.New(new NullReferenceException());
        return model;
    }
    ...
    public async Task<Fin<int>> UpdateAsync(PhoneNumberModel newModel, PhoneNumberModel oldModel, CancellationToken token = default)
    {
    ...
    public async Task<Fin<PhoneNumberModel>> DeleteAsync(int id, CancellationToken token = default)
    {
        var lastCopy = await _repository.TryGetByIdAsync(id, token);
        if (((PhoneNumberModel)lastCopy).IsNull()) return Error.New(new NullReferenceException());
