...
public class EducationDegreeModelSQLServerRepository : IEducationDegreeModelRepository
{
    ...
    public EducationDegreeModelSQLServerRepository(IServiceProvider serviceProvider)
    {
    ...
    private int GetNextTicket(ResumeContext ctx)
    {
        ...
        lock (_lock)
        {
            var models = ctx.EducationDegrees.IgnoreQueryFilters();
    ...
    public async Task<int> GetQueryTotal(GetAllEducationDegreeModelsOptions options)
    {
        ...
        {
            var ctx = scope.ServiceProvider.GetRequiredService<ResumeContext>();
            if(!ctx.EducationDegrees.Any()) return 0;
            int count = ctx.EducationDegrees.GroupBy(x=>x.CommonIdentity).Count();
            return count;
        }
    }
        ...
        if (ctx == null)
        {
            return Error.New(
                new NullReferenceException(
                    "The provider returned a null DbContext while trying to perform an EducationDegree Model Service"));
        }
        if (ctx.EducationDegrees is null)
        {
            return Error.New(
                new NullReferenceException(
                    "The provider DbSet of EducationDegree Models was NULL or uninitialized"
            ));
        }
        return ctx;
    }
    ...
    public async Task<int> CreateAsync(EducationDegreeModel model, ResumeContext ctx, CancellationToken token = default)
    {
		ctx.Attach(model.Institution);
    ...
    public async Task<IEnumerable<EducationDegreeModel>> GetAllAsync(GetAllEducationDegreeModelsOptions options, ResumeContext ctx, CancellationToken token = default)
    {
        var allDBEntries = (await ctx.EducationDegrees.ToListAsync()).GroupByAndFindLatest();
    ...
    public async Task<EducationDegreeModel> GetByIdAsync(int id, ResumeContext ctx, CancellationToken token = default)
    {
        var entries = (await ctx.EducationDegrees.ToListAsync()).GroupByAndFindLatest();
    ...
    public async Task<int> UpdateAsync(EducationDegreeModel model, ResumeContext ctx, CancellationToken token = default)
    {
        await ctx.EducationDegrees.AddAsync(model);
        return await ctx.SaveChangesAsync();
    }

    public async Task<Fin<int>> TryUpdateToHiddenAsync(EducationDegreeModel model, CancellationToken token)
    {
        try
        {
            using (var scope = _serviceProvider.CreateScope())
    ...
    public async Task<int> UpdateToHiddenAsync(EducationDegreeModel oldModel, ResumeContext ctx, CancellationToken token)
    {
        return await ctx.EducationDegrees
