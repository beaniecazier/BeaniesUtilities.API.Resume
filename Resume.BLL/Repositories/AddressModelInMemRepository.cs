...
public class AddressModelInMemRepository : IAddressModelRepository
{
    ...
    {
        if ( _addresses.Count <= 0) return 0;
        int count = _addresses.GroupBy(x => x.CommonIdentity).Count();
        return count;
    }
    ...
    {
        if(!options.HasFilters) return Task.FromResult(_addresses.GroupByAndFindLatest(options.AllowHidden??false, options.AllowDeleted??false));
