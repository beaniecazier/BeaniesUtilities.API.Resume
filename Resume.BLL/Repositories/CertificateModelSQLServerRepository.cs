...
public class CertificateModelSQLServerRepository : ICertificateModelRepository
{
    ...
    public CertificateModelSQLServerRepository(IServiceProvider serviceProvider)
    {
    ...
    private int GetNextTicket(ResumeContext ctx)
    {
        ...
        lock (_lock)
        {
            var models = ctx.Certificates.IgnoreQueryFilters();
    ...
    public async Task<int> GetQueryTotal(GetAllCertificateModelsOptions options)
    {
        ...
        {
            var ctx = scope.ServiceProvider.GetRequiredService<ResumeContext>();
            if(!ctx.Certificates.Any()) return 0;
            int count = ctx.Certificates.GroupBy(x=>x.CommonIdentity).Count();
            return count;
        }
    }
        ...
        if (ctx == null)
        {
            return Error.New(
                new NullReferenceException(
                    "The provider returned a null DbContext while trying to perform an Certificate Model Service"));
        }
        if (ctx.Certificates is null)
        {
            return Error.New(
                new NullReferenceException(
                    "The provider DbSet of Certificate Models was NULL or uninitialized"
            ));
        }
        return ctx;
    }
    ...
    public async Task<int> CreateAsync(CertificateModel model, ResumeContext ctx, CancellationToken token = default)
    {
		ctx.Attach(model.Issuer);
    ...
    public async Task<IEnumerable<CertificateModel>> GetAllAsync(GetAllCertificateModelsOptions options, ResumeContext ctx, CancellationToken token = default)
    {
        var allDBEntries = (await ctx.Certificates.ToListAsync()).GroupByAndFindLatest();
    ...
    public async Task<CertificateModel> GetByIdAsync(int id, ResumeContext ctx, CancellationToken token = default)
    {
        var entries = (await ctx.Certificates.ToListAsync()).GroupByAndFindLatest();
    ...
    public async Task<int> UpdateAsync(CertificateModel model, ResumeContext ctx, CancellationToken token = default)
    {
        await ctx.Certificates.AddAsync(model);
        return await ctx.SaveChangesAsync();
    }

    public async Task<Fin<int>> TryUpdateToHiddenAsync(CertificateModel model, CancellationToken token)
    {
        try
        {
            using (var scope = _serviceProvider.CreateScope())
    ...
    public async Task<int> UpdateToHiddenAsync(CertificateModel oldModel, ResumeContext ctx, CancellationToken token)
    {
        return await ctx.Certificates
