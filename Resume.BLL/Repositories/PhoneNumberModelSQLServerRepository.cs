...
public class PhoneNumberModelSQLServerRepository : IPhoneNumberModelRepository
{
    ...
    public PhoneNumberModelSQLServerRepository(IServiceProvider serviceProvider)
    {
    ...
    private int GetNextTicket(ResumeContext ctx)
    {
        ...
        lock (_lock)
        {
            var models = ctx.PhoneNumbers.IgnoreQueryFilters();
    ...
    public async Task<int> GetQueryTotal(GetAllPhoneNumberModelsOptions options)
    {
        ...
        {
            var ctx = scope.ServiceProvider.GetRequiredService<ResumeContext>();
            if(!ctx.PhoneNumbers.Any()) return 0;
            int count = ctx.PhoneNumbers.GroupBy(x=>x.CommonIdentity).Count();
            return count;
        }
    }
        ...
        if (ctx == null)
        {
            return Error.New(
                new NullReferenceException(
                    "The provider returned a null DbContext while trying to perform an PhoneNumber Model Service"));
        }
        if (ctx.PhoneNumbers is null)
        {
            return Error.New(
                new NullReferenceException(
                    "The provider DbSet of PhoneNumber Models was NULL or uninitialized"
            ));
        }
        return ctx;
    }
    ...
    public async Task<int> CreateAsync(PhoneNumberModel model, ResumeContext ctx, CancellationToken token = default)
    {
    ...
    public async Task<IEnumerable<PhoneNumberModel>> GetAllAsync(GetAllPhoneNumberModelsOptions options, ResumeContext ctx, CancellationToken token = default)
    {
        var allDBEntries = (await ctx.PhoneNumbers.ToListAsync()).GroupByAndFindLatest();
    ...
    public async Task<PhoneNumberModel> GetByIdAsync(int id, ResumeContext ctx, CancellationToken token = default)
    {
        var entries = (await ctx.PhoneNumbers.ToListAsync()).GroupByAndFindLatest();
    ...
    public async Task<int> UpdateAsync(PhoneNumberModel model, ResumeContext ctx, CancellationToken token = default)
    {
        await ctx.PhoneNumbers.AddAsync(model);
        return await ctx.SaveChangesAsync();
    }

    public async Task<Fin<int>> TryUpdateToHiddenAsync(PhoneNumberModel model, CancellationToken token)
    {
        try
        {
            using (var scope = _serviceProvider.CreateScope())
    ...
    public async Task<int> UpdateToHiddenAsync(PhoneNumberModel oldModel, ResumeContext ctx, CancellationToken token)
    {
        return await ctx.PhoneNumbers
