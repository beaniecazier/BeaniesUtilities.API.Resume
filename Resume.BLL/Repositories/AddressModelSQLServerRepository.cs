...
public class AddressModelSQLServerRepository : IAddressModelRepository
{
    ...
    private int GetNextTicket(ResumeContext ctx)
    {
        ...
        lock (_lock)
        {
            var models = ctx.Addresses.IgnoreQueryFilters();
    ...
    public async Task<int> GetQueryTotal(GetAllAddressModelsOptions options)
    {
        using(var scope = _serviceProvider.CreateScope())
        {
            var ctx = scope.ServiceProvider.GetRequiredService<ResumeContext>();
            if(!ctx.Addresses.Any()) return 0;
            int count = ctx.Addresses.GroupBy(x=>x.CommonIdentity).Count();
            return count;
        }
    }
    ...
    public async Task<IEnumerable<AddressModel>> GetAllAsync(GetAllAddressModelsOptions options, ResumeContext ctx, CancellationToken token = default)
    {
        var allDBEntries = (await ctx.Addresses.ToListAsync()).GroupByAndFindLatest();
