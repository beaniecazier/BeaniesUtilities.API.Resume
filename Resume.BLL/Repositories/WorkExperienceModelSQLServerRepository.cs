...
public class WorkExperienceModelSQLServerRepository : IWorkExperienceModelRepository
{
    ...
    public WorkExperienceModelSQLServerRepository(IServiceProvider serviceProvider)
    {
    ...
    private int GetNextTicket(ResumeContext ctx)
    {
        ...
        lock (_lock)
        {
            var models = ctx.WorkExperiences.IgnoreQueryFilters();
    ...
    public async Task<int> GetQueryTotal(GetAllWorkExperienceModelsOptions options)
    {
        using(var scope = _serviceProvider.CreateScope())
        {
            var ctx = scope.ServiceProvider.GetRequiredService<ResumeContext>();
            if(!ctx.WorkExperiences.Any()) return 0;
            int count = ctx.WorkExperiences.GroupBy(x=>x.CommonIdentity).Count();
            return count;
        }
    }
        ...
        if (ctx == null)
        {
            return Error.New(
                new NullReferenceException(
                    "The provider returned a null DbContext while trying to perform an WorkExperience Model Service"));
        }
        if (ctx.WorkExperiences is null)
        {
            return Error.New(
                new NullReferenceException(
                    "The provider DbSet of WorkExperience Models was NULL or uninitialized"
            ));
        }
        return ctx;
    }
    ...
    public async Task<int> CreateAsync(WorkExperienceModel model, ResumeContext ctx, CancellationToken token = default)
    {
		foreach(var item in model.TechUsed) ctx.Attach(item);
    ...
    public async Task<IEnumerable<WorkExperienceModel>> GetAllAsync(GetAllWorkExperienceModelsOptions options, ResumeContext ctx, CancellationToken token = default)
    {
        var allDBEntries = (await ctx.WorkExperiences.ToListAsync()).GroupByAndFindLatest();
    ...
    public async Task<WorkExperienceModel> GetByIdAsync(int id, ResumeContext ctx, CancellationToken token = default)
    {
        var entries = (await ctx.WorkExperiences.ToListAsync()).GroupByAndFindLatest();
    ...
    public async Task<int> UpdateAsync(WorkExperienceModel model, ResumeContext ctx, CancellationToken token = default)
    {
        await ctx.WorkExperiences.AddAsync(model);
        return await ctx.SaveChangesAsync();
    }

    public async Task<Fin<int>> TryUpdateToHiddenAsync(WorkExperienceModel model, CancellationToken token)
    {
        try
        {
            using (var scope = _serviceProvider.CreateScope())
    ...
    public async Task<int> UpdateToHiddenAsync(WorkExperienceModel oldModel, ResumeContext ctx, CancellationToken token)
    {
        return await ctx.WorkExperiences
